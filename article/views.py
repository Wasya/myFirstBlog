...
def article(request, article_id=1, comments_page_no=1):
    comment_form = CommentForm
    all_comments = Comments.objects.filter(comments_article_id = article_id)
    current_comment_page = Paginator(all_comments,4)
    args = {}
    args.update(csrf(request))
    args['article'] = Article.objects.get(id = article_id)
    ###args['comments'] =  Comments.objects.filter(comments_article_id = article_id)
    args['comments'] =  current_comment_page.page(comments_page_no)
